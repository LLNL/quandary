...
class TimeStepper{
  protected:
    ...
    int mpirank_optim;
    int mpisize_optim;
    int n_start;    // first time index for PinT local time-stepping
    int n_stop;     // last time index for PinT local time-stepping
  ...
  public: 
    ...
    TimeStepper(); 
    TimeStepper(MapParam config, MasterEq* mastereq_, int ntime_, double total_time_, Output* output_, bool storeFWD_, MPI_Comm comm_optim_); 
    virtual ~TimeStepper(); 
...
class ExplEuler : public TimeStepper {
  Vec stage;
  public:
    ExplEuler(MapParam config, MasterEq* mastereq_, int ntime_, double total_time_, Output* output_, bool storeFWD_, MPI_Comm comm_optim_);
    ~ExplEuler();
...
class ImplMidpoint : public TimeStepper {
  ...
  public:
    ImplMidpoint(MapParam config, MasterEq* mastereq_, int ntime_, double total_time_, LinearSolverType linsolve_type_, int linsolve_maxiter_, Output* output_, bool storeFWD_, MPI_Comm comm_optim_);
    ~ImplMidpoint();
...
class CompositionalImplMidpoint : public ImplMidpoint {
  ...
  public:
    CompositionalImplMidpoint(MapParam config, int order_, MasterEq* mastereq_, int ntime_, double total_time_, LinearSolverType linsolve_type_, int linsolve_maxiter_, Output* output_, bool storeFWD_, MPI_Comm comm_optim_);
