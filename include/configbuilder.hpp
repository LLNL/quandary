...
struct OptimTargetConfig {
  ...
  std::optional<GateType> gate_type; ///< Gate type (if target_type is GATE)
  std::optional<std::string> filename; ///< Filename (if target_type is FILE)
  std::vector<int> levels; ///< Pure state levels (if target_type is PURE)
};
