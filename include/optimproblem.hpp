...
class OptimProblem {
   ...
   * @param x_is_control_ Flag to switch between optimizing controls (true) or UDE models (false)
   * @param output_ Pointer to output handler
   * @param quietmode Flag for quiet operation (default: false)
   */
  OptimProblem(Config config, TimeStepper* timestepper_, MPI_Comm comm_init_, MPI_Comm comm_optim, int ninit_, bool x_is_control_, Output* output_, bool quietmode=false);
  ...
  /**
   * @brief Retrieves the UDE objective loss function value.
   *
   * @return double UDE Loss objective function value
   */
