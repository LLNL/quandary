...
typedef struct {
  ...
  Learning* learning; ///< UDE learning module
...
class MasterEq{
  ...
  public:
    ...
    Learning* learning;   ///< Pointer to UDE learning class
  ...
  public:
    ...
    /**
     * @brief  Set the oscillators control function parameters from global design vector x
     * 
     * This function does pass the global design vector x into the Osciallator's storage for contorl parameters
     * 
     * @param x Global design vector
     */ 
    void setControlAmplitudes(const Vec x);

    /**
     * @brief If learning: Set control amplitudes from training data. Note: pulse_num is global. 
     * 
     * @param pulse_num Pulse numbe identifier
     */
 ...
 */
void compute_dRHS_dParams_sparsemat(const double t,const Vec x,const Vec x_bar, const double alpha, Vec grad, std::vector<int>& nlevels, IS isu, IS isv, std::vector<Mat>& Ac_vec, std::vector<Mat>& Bc_vec, Vec aux, Oscillator** oscil_vec, bool x_is_control, Learning* learning);
