...
Data::Data(Config config, MPI_Comm comm_optim_, MPI_Comm comm_init_, int ninit_, std::vector<int> nlevels_, LindbladType lindbladtype_) {
  ...
  } else if (data_identifier.compare("Tant3Pop")==0) {
    loadData_Tant3Pop(data_names);
  } else {
    printf("Wrong data identifier: Needs prefix 'synthetic' (for synthetic density matrix data), or 'syntheticPop' (for synthetic population data), or 'Tant3Pop' (for Tant 3-level population data).\n");
    exit(1);
  }
...
// loadData_Tant3Pop
//
void Data::loadData_Tant3Pop(std::vector<std::vector<std::string>> &data_names){
  // NOTE: This function is specialized to reading 3-level population data generated by the "Tant" QuDIT device.
  densityData = false;
  ...
  for (int ipulse_local = 0; ipulse_local < npulses_local; ipulse_local++){
    ...
    for (int iinit_local =0; iinit_local < ninit_local; iinit_local++){
      ...
      // NOTE: there is no header line in these data files
      std::string oneLine;

      // time levels are hard-coded
      tstart = 16.0; // [ns]
      dt = 4.0; // [ns]
      double pop[3]; // expecting 3 population values per line

      while (std::getline(infile_pop, oneLine)) // could specify ',' as the delimiter instead of the default ' ' 
      {
        // std::cout << "loadData_Tant3Pop: count = " << count << " oneLine = " << oneLine << std::endl;
        // Figure out time level
        time_pop = tstart + count*dt;
        // Break if exceeding the requested time domain length
        if (time_pop > tstop)  {
          printf("Stopping data at %1.8f > %1.8f \n", time_pop, tstop);
          break;
        ...
        std::stringstream sst(oneLine);
        if (sst >> pop[0] >> pop[1] >> pop[2]) {
          //printf("Read pop[0]: %.20e\n", pop[0]);
        } else {
          std::cout << "Failed to read 3 populations from last line: " << oneLine << std::endl;
          exit(-1);
        }
        ...
        Vec state;
        VecCreate(PETSC_COMM_WORLD, &state);
        VecSetSizes(state, PETSC_DECIDE, 3); // 3 populations per line
        VecSetFromOptions(state);

        for (int i=0; i<3; i++) { 
          VecSetValue(state, i, pop[i], INSERT_VALUES);
        }
        VecAssemblyBegin(state);
        VecAssemblyEnd(state);
        ...
        data[ipulse_local][iinit_local].push_back(state);
        count+=1;
        // time_prev = time_pop; // not needed because the time step is hard-coded
      }
