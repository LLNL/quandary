...
double OptimProblem::evalF(const Vec x) {
  ...
  for (int iinit = 0; iinit < ninit_local; iinit++) {
    ...
    finalstate = timestepper->solveODE(iinit, initid, rho_t0);
...
void OptimProblem::evalGradF(const Vec x, Vec G){
  ...
  for (int iinit = 0; iinit < ninit_local; iinit++) {
    ...
    if (timestepper->mastereq->lindbladtype != LindbladType::NONE) {
      ...
      timestepper->solveAdjointODE(iinit, initid, rho_t0_bar, finalstate, obj_weights[iinit] * gamma_penalty, obj_weights[iinit]*gamma_penalty_dpdm, obj_weights[iinit]*gamma_penalty_energy);
