...
double OptimProblem::evalF(const Vec x) {
  ...
  objective = obj_cost + obj_regul + obj_penal + obj_penal_energy;

  /* Output */
  if (mpirank_world == 0) {
    std::cout<< "Objective = " << std::scientific<<std::setprecision(14) << obj_cost << " + " << obj_regul << " + " << obj_penal << " + " << obj_penal_energy << std::endl;
...
void OptimProblem::evalGradF(const Vec x, Vec G){
  ...
  for (int iinit = 0; iinit < ninit_local; iinit++) {
    ...
    if (timestepper->mastereq->lindbladtype != LindbladType::NONE) {
      ...
      timestepper->solveAdjointODE(initid, rho_t0_bar, finalstate, obj_weights[iinit] * gamma_penalty, obj_weights[iinit]* gamma_penalty_energy);
  ...
  if (mpirank_world == 0 && !quietmode) {
    std::cout<< "Objective = " << std::scientific<<std::setprecision(14) << obj_cost << " + " << obj_regul << " + " << obj_penal << 
    " + " << obj_penal_energy << std::endl;
    std::cout<< "Fidelity = " << fidelity << std::endl;
  }
}
...
PetscErrorCode TaoMonitor(Tao tao,void*ptr){
  ...
  ctx->output->writeOptimFile(f, gnorm, deltax, F_avg, obj_cost, obj_regul, obj_penal, obj_penal_energy);
  ...
  if (ctx->getMPIrank_world() == 0) {
    std::cout<< iter <<  ": Objective = " << std::scientific<<std::setprecision(14) << obj_cost << " + " << obj_regul << " + " << obj_penal << " + " << obj_penal_energy;
