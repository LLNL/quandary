...
void MasterEq::apply_linearized_RHS(const double t,const Vec v, const Vec xhalf, Vec xout){
  ...
  if (!usematfree) {  // Sparse-matrix application of RHS 
    ...
    for (int iosc= 0; iosc < noscillators; iosc++){
      ...
      std::vector<double> vdir(oscil_vec[iosc]->getNParams());
      ...
      // TODO: PLUS OR MINUS??
      // u += q Ac u - p Bc v
      MatMult(Ac_vec[iosc], uhalf, aux); VecAXPY(uout,  dqv, aux);
      MatMult(Bc_vec[iosc], vhalf, aux); VecAXPY(uout, -dpv, aux);
      // v += q Ac v + p Bc u
      MatMult(Ac_vec[iosc], vhalf, aux); VecAXPY(vout,  dqv, aux);
      MatMult(Bc_vec[iosc], uhalf, aux); VecAXPY(vout,  dpv, aux);
    ...
    printf("TODO: Matrix-free version of linearized RHS.\n");
    exit(1);
  }
...
void MasterEq::apply_linearized_RHS_transpose(const double t,const Vec v,  const Vec xhalf, Vec xout){
  ...
  if (!usematfree) {  // Sparse-matrix application of RHS 
    ...
    for (int iosc= 0; iosc < noscillators; iosc++){
      ...
      // TODO: PLUS OR MINUS??
      // u += q Ac^T u + p Bc^T v
      MatMultTranspose(Ac_vec[iosc], uhalf, aux); VecAXPY(uout,  dqv, aux);
      MatMultTranspose(Bc_vec[iosc], vhalf, aux); VecAXPY(uout,  dpv, aux);
      // v += q Ac^T v - p Bc^T u
      MatMultTranspose(Ac_vec[iosc], vhalf, aux); VecAXPY(vout,  dqv, aux);
      MatMultTranspose(Bc_vec[iosc], uhalf, aux); VecAXPY(vout, -dpv, aux);
