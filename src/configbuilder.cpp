...
std::vector<ControlSegmentConfig> ConfigBuilder::convertFromString<std::vector<ControlSegmentConfig>>(const std::string& str) {
  ...
  while (i < parts.size()) {
    ...
    // Validate minimum parameter count per control type (use lenient validation)
    size_t min_params = 1; // Most control types need at least 1 parameter
    switch (segment.control_type) {
      case ControlType::STEP:
        min_params = 1; // Only require first parameter, others can be defaulted
        break;
      case ControlType::BSPLINE:
      ...
      case ControlType::BSPLINEAMP:
        min_params = 1; // Only require num_basis_functions, scaling can be defaulted
        break;
