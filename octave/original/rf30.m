...
%
function  [f] = rf30(t, param)
  D = size(param.pcof,1);
  if (D != 30)
    printf("ERROR: rf30 only works when pcof has 30 elements\n");
    f=-999;
    return;
  end
  f = 0;
...
# adding a period T/8 wavelet, centered at (15/16)*T
  ...
  envelope = 64*(tau >= -0.5 & tau <= 0.5).*(0.5 + tau).^3 .* (0.5 - tau).^3;
# from state 1 (ground) to state 2
  f = f + param.pcof(26) * envelope .*cos(param.d_omega(1)*t);
# state 2 to 3
  f = f + param.pcof(27) * envelope .*cos(param.d_omega(2)*t);
# state 3 to 4
  f = f + param.pcof(28) * envelope .*cos(param.d_omega(3)*t);
  # state 4 to 5
  f = f + param.pcof(29) * envelope .*cos(param.d_omega(4)*t);
# state 5 to 6
  f = f + param.pcof(30) * envelope .*cos(param.d_omega(5)*t);
