...
%
function  [f] = rf15(t, param)
  D = size(param.pcof,1);
  if (D != 15)
    printf("ERROR: rf15 only works when pcof has 15 elements\n");
    f=-999;
    return;
  end
  f = 0;

# ONLY using period T/2 wavelets, centered at (0.25, 0.5, 0.75)*T
  tp = 0.5*param.T;
