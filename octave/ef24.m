...
%
function  [f] = ef24(t, param)
  D = size(param.pcof,1);
  if (D < 24)
    printf("ERROR: ef24 only works when pcof has at least 24 elements\n");
    f=-999;
    return;
  end
  f = zeros(4,length(t));
...
# period T/4 wavelets, centered at (0.75, 0.875)*T
  ...
  envelope = 64*(tau >= -0.5 & tau <= 0.5).*(0.5 + tau).^3 .* (0.5 - tau).^3;
# from state 1 (ground) to state 2
  f(1,:) = f(1,:) + param.pcof(17) * envelope;
# state 2 to 3
  f(2,:) = f(2,:) + param.pcof(18) * envelope;
# state 3 to 4
  f(3,:) = f(3,:) + param.pcof(19) * envelope;
# state 4 to 5
  f(4,:) = f(4,:) + param.pcof(20) * envelope;
