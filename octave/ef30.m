...
%
function  [f] = ef30(t, param)
  D = size(param.pcof,1);
  if (D != 30)
    printf("ERROR: ef30 only works when pcof has 30 elements\n");
    f=-999;
    return;
  end
  f = zeros(5,length(t));
...
# state 5 to 6
  ...
  envelope = 64*(tau >= -0.5 & tau <= 0.5).*(0.5 + tau).^3 .* (0.5 - tau).^3;
# from state 1 (ground) to state 2
  f(1,:) = f(1,:) + param.pcof(26) * envelope;
# state 2 to 3
  f(2,:) = f(2,:) + param.pcof(27) * envelope;
# state 3 to 4
  f(3,:) = f(3,:) + param.pcof(28) * envelope;
# state 4 to 5
  f(4,:) = f(4,:) + param.pcof(29) * envelope;
# state 5 to 6
  f(5,:) = f(5,:) + param.pcof(30) * envelope;
