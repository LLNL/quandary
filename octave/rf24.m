...
%
function  [f] = rf24(t, param)
  D = size(param.pcof,1);
  if (D != 24)
    printf("ERROR: rf24 only works when pcof has 24 elements\n");
    f=-999;
    return;
  end
  f = 0;
...
# period T/4 wavelets, centered at (0.75, 0.875)*T
  ...
  envelope = 64*(tau >= -0.5 & tau <= 0.5).*(0.5 + tau).^3 .* (0.5 - tau).^3;
# from state 1 (ground) to state 2
  f = f + param.pcof(17) * envelope .*cos(param.d_omega(1)*t);
# state 2 to 3
  f = f + param.pcof(18) * envelope .*cos(param.d_omega(2)*t);
# state 3 to 4
  f = f + param.pcof(19) * envelope .*cos(param.d_omega(3)*t);
# state 4 to 5
  f = f + param.pcof(20) * envelope .*cos(param.d_omega(4)*t);
