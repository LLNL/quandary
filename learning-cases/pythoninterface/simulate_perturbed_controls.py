...
pcof0_filename = os.getcwd() + "/opt_params.dat"  # absolute path!
# pcof0_filename = os.getcwd() + "/shift3_params.dat"  # absolute path!
...
quandary = Quandary(freq01=freq01, Jkl=Jkl, rotfreq=rotfreq, T=T, targetgate=unitary, verbose=verbose, rand_seed=rand_seed, spline_order=spline_order, nsplines=nsplines, gamma_variation=gamma_variation, control_enforce_BC=control_enforce_BC, pcof0_filename=pcof0_filename, Pmin=Pmin) 


# Execute quandary in simulatio mode
t, pt, qt, infidelity, expectedEnergy, population = quandary.simulate(datadir="./run_dir")
...
amp = 1.1 # units for amp are MHz

# perturb the time-series of the control functions by random numbers and store the results in lists
pt_pert = []
qt_pert = []
