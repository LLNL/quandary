...
int main(int argc,char **argv)
{
  ...
  */
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     ...
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ierr = MatCreateShell(PETSC_COMM_SELF,PETSC_DECIDE,PETSC_DECIDE,1,1,&appctx,&A);CHKERRQ(ierr);
  ierr = MatShellSetOperation(A, MATOP_MULT, (void(*)(void)) UserMult);CHKERRQ(ierr);
...
PetscErrorCode UserMult(Mat mat, Vec x, Vec y)
{
  ...
  ierr = MatShellGetContext(mat, appctx);CHKERRQ(ierr);
  ierr = VecCreateSeq(PETSC_COMM_SELF,1,&z);CHKERRQ(ierr);
  ierr = VecSet(z, 0);CHKERRQ(ierr);
  ierr = VecWAXPY(y, appctx->lambda, x, z);CHKERRQ(ierr);
  return 0;
}
