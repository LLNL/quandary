from quandary import Quandary, plot_expectedEnergy, plot_pulse
...
def test_example(request):
    def mock_show(*args, **kwargs):
        """Mock plt.show() to accept any arguments and do nothing."""
        pass

    with patch("builtins.input", return_value=""):  # Mock input() to simulate pressing Enter
        with patch("matplotlib.pyplot.show", mock_show):  # Mock plt.show() to bypass blocking
            cnot(request)
...
def cnot(request):
    ...
    # Make sure to pass all of the above to the corresponding fields, compare help(Quandary)!
    quandary = Quandary(
        freq01=freq01,
        Jkl=Jkl,
        rotfreq=rotfreq,
