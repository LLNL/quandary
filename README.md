...
## Changelog
### v1.
* In addition to solving Lindblad's master equation for the density matrix, Quandary can now also solve Schroedinger's equation for the state vector describing closed quantum systems, if no collapse operators are present. 
* Rename the config option for objective function: Previously `Jhilbertschmidt`, now `Jtrace`

### v0. 
Initial version. Simulating and optimizing with Lindblad's master equation for the density matrix describing open quantum systems. MPI-parallel linear algebra, MPI-parallel initial conditions. 
